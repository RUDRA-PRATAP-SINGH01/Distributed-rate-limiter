...
Each routed request: 1× `ListGateways` (N HGETALL), 1× circuit Allow per attempt, 1× upstream HTTP, 1× outcome Lua. Failover multiplies upstream attempts — cap `ROUTING_MAX_FAILOVER_TRIES` to control tail latency. Weighted random is O(n) over small n (typically 3–10 gateways) — negligible vs network.
