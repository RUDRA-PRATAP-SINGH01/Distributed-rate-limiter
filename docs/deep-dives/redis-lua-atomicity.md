# Redis Lua Atomicity — Engineering Journal
...
1. **One script = one business operation** — no splitting claim vs complete across round trips without fences.
...
- **Primary CPU concentration** — hot keys serialize on one thread; Lua length matters.
- **No partial rollback across keys in Cluster** — if KEYS span slots, `CROSSSLOT` errors occur; we assume standalone or correctly tagged keys.
