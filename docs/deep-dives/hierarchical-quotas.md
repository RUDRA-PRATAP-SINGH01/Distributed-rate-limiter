...
1. Refill, check, and deduct all four levels in one Redis script.
2. Dynamic overrides: Admin can change tenant/user caps without redeploy; `AllowWithParams` accepts runtime capacities.
3. One `EVAL` per hierarchical check, same as flat token bucket.
4. Return the tightest bottleneck (minimum remaining across levels).
...
**Phase 1 (lines 12 to 49):** For each level, `HMGET tokens,last_refill`, compute refill, floor tokens, check `>= 1`, track `min_remaining`, write refilled state with `HMSET` + `EXPIRE 3600`.

**Phase 2 (lines 51 to 63):** If `allowed == 1`, decrement each level by 1; else `remaining = 0`.
...
- Sliding window at endpoint-only granularity would need a different script; not mixed today.
...
4. Four HMGET/HMSET loops on a slow primary block other commands; watch `slowlog`.
