# Chaos Testing — Engineering Journal
...
- **Windows PowerShell + Python** — split scripts for OS convenience; CI must run on compatible runner.
- **Compose-specific names** — `rate-sidecar`, network labels — break if compose file renames services.
- **Not full Netflix Chaos Monkey** — no random production injection; staging only.
...
2. **Port conflict** — local `go run` on 8080 blocks compose — README warns to stop first.
