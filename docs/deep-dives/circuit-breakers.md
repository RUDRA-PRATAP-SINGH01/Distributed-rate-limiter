# Circuit Breakers — Engineering Journal
...
**Router integration** (`router.go` lines 133–160):
...
- **Half-open probe cap** — too low slows recovery detection; too high loads dying service.
...
3. **Latency-only open** — latency spike count without hard failure — can open on slow but successful deps.
4. **Allow/Record race** — two sidecars allow in half-open; both record — probe budget limits blast radius.
5. **Reset during incident** — manual `Reset` before root cause fixed → immediate relapse.
