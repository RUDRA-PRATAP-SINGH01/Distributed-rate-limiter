...
**New claim** (`claim.lua` lines 25 to 34): writes `fence_token` with `status=processing`.

**Reclaim after expiry** (lines 55 to 66): overwrites `fence_token` with new ARGV[5]. old holder invalidated.

**Complete path** (`complete.lua` lines 29 to 32):
...
1. Expected; log and drop. better than corrupt cache.
2. Missing fence in capturer bug: Complete always fails; stuck `processing` until TTL. monitor in-progress keys.
3. Astronomically unlikely; ignored.
4. Ops sets wrong fence. breaks complete; don't hand-edit idem keys.
