...
- The same user cannot consume more than their budget because they hit different pods.
- A limit check cannot add hundreds of milliseconds to every API call.
...
### API gateway as the only enforcement point
...
### Fail-open everywhere when Redis is down
...
**Central limiter adds a network hop.** I pay ~1 to 5 ms (LAN) per uncached check in exchange for one implementation of hierarchical limits and one place to attach audit. Sidecar denial cache (default 30 ms TTL) and singleflight amortize this on hot keys.
